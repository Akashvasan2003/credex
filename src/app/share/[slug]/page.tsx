...
export default function SharePage() {
  ...
  useEffect(() => {
    ...
    async function loadAudit() {
      const { data } = await supabase.from("audits").select("*").eq("share_slug", params.slug).single();
    ...
    return (
      <div className="min-h-screen bg-[#080808] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white/40 text-sm">Loading shared audit...</p>
        </div>
      </div>
    );
  }
  ...
  return <ResultsPage audit={audit} isShared={true} />;
}
